<template>
    ...
    <el-row :gutter="10">
      ...
      <el-col :span="4">
        <el-form-item label="作者">
          <el-input
            v-model="queryDto.author"
            placeholder="请输入要搜索的作者"
          ></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="6">
  ...
  <el-table :data="list" @selection-change="handleSelectionChange">
    ...
    <el-table-column label="审核" align="center" width="280" #default="scope">
      <el-button
        plain
        :disabled="scope.row.approvalStatus !== 2"
        type="primary"
        size="small"
  ...
  >
    ...
    <el-button type="primary" @click="rejectSubmit" class="rejectRbtn"
      >确认驳回
    </el-button>
  </el-dialog>
  <!--内容查看窗体-->
...
const total = ref(0)
const queryDto = ref({ title: '', content: '', author: '', tid: '' })
onMounted(async () => {
  await fetchData()
})
...
const pass = async val => {
  const { code } = await ApprovePass(val.id)
  if (code === 200) {
...
const reject = val => {
  ...
  rejectReason.value.id = val.id
}
const rejectSubmit = async () => {
  const { code } = await ApproveReject(rejectReason.value)
  if (code === 200) {
